...
document.addEventListener('DOMContentLoaded', function() {
      ...
      if (!window.zappyContactFormLoaded) {
        ...
        try {
          ...
          const response = await fetch('http://localhost:5001/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '24107025-355c-46de-a72e-055ffe845c5b',
              name: formFields.name || formFields.fullName || formFields.userName || '',
              email: formFields.email || formFields.userEmail || '',
              subject: formFields.subject || 'Contact Form Submission',
              message: formFields.message || formFields.comments || formFields.description || '',
  ...
  document.querySelectorAll('.bento-item, .product-card, .mission-card').forEach(el => {
    el.style.opacity = '0';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מתוק לי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
